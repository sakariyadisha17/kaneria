            ...
            <section id="column">
                <div class="row">
                    <div class="col-12">
                        ...
                        <form method="POST" action="{{ route('reception.letterhead.save', ['id' => $patient->id]) }}">
                            @csrf
                                    ...
                                    <div class="col-md-9">
                                        ...
                                        <select id="diagnosis" name="diagnosis[]" class="form-control" onchange="fetchDiagnosisDetails()" multiple>
                                            ...
                                            @foreach($diagnosis as $diagno)
                                                <option value="{{ $diagno->id }}" 
                                                    {{ in_array($diagno->id, json_decode($letterhead->diagnosis ?? '[]')) ? 'selected' : '' }}>
                                                    {{ $diagno->name }}
                                                </option>
                                            @endforeach
                                        </select>
    ...
    $(document).ready(function () {
        $('#medicinesList').on('click', '.deleteMedicineBtn_delete', function () {
            ...
            if (confirm('Are you sure you want to delete this medicine?')) {
                $.ajax({
                    url: `{{ url('receptionist/appointments/delete-letterhead-medicine/') }}/${medicineId}`,  
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}' 
                    },
...
$.ajax({
    url: '{{ route("reception.letterhead.save", ":id") }}'.replace(':id', '{{ $patient->id }}'),
    method: 'POST',
