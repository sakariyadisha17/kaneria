        ...
        <div class="row">
          <div class="col-12">
            <div class="text-right mb-3">
              <a href="{{ url('medical_officer/patient_dose') }}" class="btn btn-info">
              ...
              <div id="vitalTableContainer" class="mt-3">
                <div class="card">
                  ...
                  <div class="card-body" style="overflow-x: auto; -webkit-overflow-scrolling: touch;">
                    <table class="table table-bordered" id="vitalTable" style="min-width: 900px; width: 100%; border-collapse: collapse;">
                      <thead>
                        <tr>
                          <th style="white-space: nowrap; padding: 8px;">Date & Time</th>
                          <th style="white-space: nowrap; padding: 8px;">BP</th>
                          <th style="white-space: nowrap; padding: 8px;">Pulse</th>
                          <th style="white-space: nowrap; padding: 8px;">SPO2</th>
                          <th style="white-space: nowrap; padding: 8px;">Temp</th>
                          <th style="white-space: nowrap; padding: 8px;">ECG</th>
                          <th style="white-space: nowrap; padding: 8px;">RBS</th>
                          <th style="white-space: nowrap; padding: 8px;">Report</th>
                          <th style="white-space: nowrap; padding: 8px;">Complaint</th>
                          <th style="white-space: nowrap; padding: 8px;">Patient Note</th>
                        </tr>
                      </thead>
                      <tbody>
                        @foreach($vital as $v)
                          <tr data-date="{{ \Carbon\Carbon::parse($v->datetime)->format('Y-m-d') }}">
                            <td style="white-space: nowrap; padding: 6px;">{{ \Carbon\Carbon::parse($v->datetime)->format('d-m-Y H:i') }}</td>
                            <td style="padding: 6px;">{{ $v->bp }}</td>
                            <td style="padding: 6px;">{{ $v->pulse }}</td>
                            <td style="padding: 6px;">{{ $v->spo2 }}</td>
                            <td style="padding: 6px;">{{ $v->temp }}</td>
                            <td style="padding: 6px;">{{ $v->ecg }}</td>
                            <td style="padding: 6px;">{{ $v->rbs }}</td>
                            <td style="padding: 6px;">{{ $v->report ?? '-' }}</td>
                            <td style="padding: 6px;">{{ $v->complaint ?? '-' }}</td>
                            <td style="padding: 6px;">{{ $v->patient_note ?? '-' }}</td>
                          </tr>
                        @endforeach
                      </tbody>
                    </table>
...
$(document).ready(function() {
  ...
  $('#printBtn').on('click', function() {
    ...
    printWindow.document.write(' table { width: 100%; border-collapse: collapse; margin: 15px auto; }');
