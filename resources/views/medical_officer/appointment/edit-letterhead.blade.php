                                                    ...
                                                    <tbody id="medicinesList">
                                                        @foreach ($medicines as $medicine)
                                                            <tr data-id="{{ $medicine->id }}">
                                                                <td data-label="Medicine">{{ $medicine->name }}</td>
                                                                <td data-label="Quantity">{{ $medicine->quantity }}</td>
                                                                <td data-label="Frequency">{{ $medicine->frequency }}</td>
                                                                <td data-label="Note">{{ $medicine->note }}</td>
                                                                <td data-label="Extra Note">{{ $medicine->extra_note }}</td>
...
<script>
    $(document).ready(function () {
        $('#medicinesList').on('click', '.deleteMedicineBtn_delete', function () {
            let row = $(this).closest('tr'); 
            let medicineId = row.data('id'); 

            // Confirm before deletion
            if (confirm('Are you sure you want to delete this medicine?')) {
                $.ajax({
                    url: `{{ url('medical_officer/appointments/delete-letterhead-medicine/') }}/${medicineId}`,  
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}' 
                    },
...
$.ajax({
    ...
    data: JSON.stringify(data),
        ...
        error: function(xhr) {
            ...
            // Redirect and pass error message to error page
            window.location.href = "/error-display-page?error=" + encodeURIComponent(message);
        }
});
