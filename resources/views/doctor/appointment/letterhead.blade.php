    ...
    <div class="content-wrapper">
        ...
        <div class="content-body">
            <section id="column">
                <div class="row">
                    <div class="col-12">
                    <div class="text-right text-right mr-2 mb-2">
                        ...
                        <form method="POST" action="{{ route('doctor.letterhead.save', ['id' => $patient->id]) }}">
                            @csrf
                            ...
                            <div class="letterhead container p-4">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-9">
                                        <h5>Diagnosis:</h5>
                                        <select id="diagnosis" name="diagnosis" class="form-control diagnosis-select" onchange="fetchDiagnosisDetails(this.value)">
                                ...
                                <div class="row mt-2">
                                    <div class="col-md-9">
                                        <h5>Diet Chart:</h5>
                                        <textarea class="form-control" name="diat" rows="2" placeholder="Diet Chart"></textarea>
                                    </div>
                                </div>
...
<script>
    ...
    var printContents = document.querySelector('.letterhead').innerHTML;

    // Open a new window for printing
    var printWindow = window.open('', '', 'height=600,width=800');
    
    // Write the structure of the new document
...
<script>
    ...
    function fetchDiagnosisDetails(diagnosisId) {
        if (diagnosisId) {
            $.ajax({
                url: '{{ route("doctor.get.diagnosis.details") }}',
                method: 'POST',
                data: {
    ...
    function fetchMedicineOptions(selectElement, selectedId = null) {
        $.ajax({
            url: '{{ route("doctor.get.medicine.list") }}',
            method: 'GET',
            success: function (response) {
...
$('#saveDetailsBtn').click(function (e) {
    ...
    $('#medicinesList tr').each(function () {
        ...
        if (medicineName && quantity && frequency) {
            medicines.push({
    ...
    if (medicines.length === 0) {
        alert('No medicines to save. Please add medicine details.');
        return;
    }
    ...
    var data = {
        ...
        diagnosis: $('select[name="diagnosis"]').val(), 
...
$.ajax({
    url: '{{ route("doctor.letterhead.save", ":id") }}'.replace(':id', '{{ $patient->id }}'),
    method: 'POST',
