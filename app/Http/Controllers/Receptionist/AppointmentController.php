...
class AppointmentController extends Controller
{
    ...
    {
        try {
            return view('receptionist.appointment.list');
        } catch (\Exception $ex) {
            Log::critical("Appointment list Page:\n" . $ex->getMessage() . " in " . $ex->getFile() . " at Line " . $ex->getLine() . ". \n[stacktrace]\n" . $ex->getTraceAsString());
            return redirect('receptionists')->with('error', 'Woops! Something went wrong. Please try again later.');
        }
    }
            ...
            ->addColumn('action', function ($patient) {
                ...
                if (auth()->user()->hasRole('Receptionist')) {
                    if ($patient->status === 'Completed'){
                        $printButton = '<a href="' . URL('receptionist/appointments/' . $patient->id . '/print') . '" ><i class="fa fa-print"></i></a>';
                   }               
                } 
                $deleteAction = '';
                if (auth()->user()->hasRole('Receptionist')) {
                ...
                $actionString = '<div style="display: flex; gap: 10px; align-items: center;">'.$letterAction . ' ' . $EditletterAction . ' ' . $uploadFileAction . ' ' . $openButton . ' ' . $deleteAction . ' ' . $printButton .'</div>';
    ...
    public function showFile($id)
    {
        ...
        return view('receptionist.appointment.showfile', compact('patientFiles', 'id'));
    }
    ...
    public function generatePDF($id)
    {
        ...
        $pdf = PDF::loadView('receptionist.appointment.generatepdf', compact('patientFiles', 'patientName', 'patientAge', 'patientPhone' ,'patientAddress'))        
    ...
    public function exportCSV(Request $request)
    {
        ...
        $columns = ['Patient ID', 'Time', 'Full Name', 'Address', 'Age', 'Phone', 'Payment Type', 'Amount', 'Booking Date', 'Status'];
    ...
    public function exportPDF(Request $request)
    {
        ...
        $pdf = PDF::loadView('receptionist.appointment.pdf', compact('patients', 'cashTotal', 'onlineTotal', 'debitTotal', 'totalAmount', 'selectedDate'))
        ->setPaper('A4', 'landscape');
    ...
    public function showletter($id)
    {
        ...
        return view('receptionist.appointment.letterhead', compact('patient','diagnosis'));
    }
    ...
    public function storeLetterhead(Request $request, $id)
    {
        ...
        try {
            // dd(json_encode($request->diagnosis));
            ...
            $letterhead->diagnosis = json_encode($request->input('diagnosis', []));
    ...
    {
        $patient = Patient::findOrFail($id); // Find the patient
        $diagnosis = Diagnosis::get(); 
        ...
        $medicines = [];
        if ($letterhead) {
            $medicines = OpdPatientMedicine::where('opd_letter_id', $letterhead->id)
                                        ->where('patient_id', $patient->id)
                                        ->get();
        }

        return view('receptionist.appointment.edit-letterhead', compact('patient', 'diagnosis', 'letterhead', 'medicines'));
    }
    ...
    public function updateLetterhead(Request $request, $id)
    {
        ...
        try {
            ...
            if ($letterhead) {
                ...
                // Get existing medicines for this letterhead to avoid duplicates
                $existingMedicines = OpdPatientMedicine::where('opd_letter_id', $letterhead->id)
                                                        ->pluck('name')
                                                        ->toArray(); // List of existing medicine names
                ...
                foreach ($medicines as $medicine) {
                    ...
                    // Check if this medicine already exists for the patient in this letterhead
                    if (in_array($medicine['medicineName'], $existingMedicines)) {
                        // If medicine already exists, skip adding
                        continue;
                    }
                ...
                // Respond with success and the updated medicines
                return response()->json([
                    'success' => true,
    ...
    public function printFile($id)
    {
        ...
        $charges = json_decode($patient->charges, true) ?? [];

        $chargeDetails = [];
        $calculatedTotal = 0;

        foreach ($charges as $chargeId) {
            $chargeDetail = Charge::find($chargeId);
            if ($chargeDetail) {
                $chargeDetails[] = [
                    'type' => $chargeDetail->type,
                    'amount' => $chargeDetail->amount,
                ];
                $calculatedTotal += $chargeDetail->amount;
            }
        }

        // Use amount, return_amount, and total_amount logic
        $amount = $patient->amount ?? 0.00;
        $returnAmount = $patient->return_amount ?? 0.00;
        $totalAmount = $patient->total_amount ?? 0.00;

        if ($returnAmount > 0) {
            $totalAmount = $amount - $returnAmount;
        } elseif ($totalAmount == 0.00) {
            $totalAmount = $amount;
        }

        // Optionally save the calculated total_amount back to the database
        if ($patient->total_amount != $totalAmount) {
            $patient->total_amount = $totalAmount;
            $patient->save();
        }

        return view('receptionist.appointment.print', compact('patient', 'id', 'chargeDetails', 'amount', 'returnAmount', 'totalAmount'));
    }
    ...
    public function print($id)
    {
        $patient = Patient::findOrFail($id);
        $diagnosis = Diagnosis::get();
        return view('receptionist.appointment.letterhead.print', compact('patient', 'diagnosis'));
    }
    ...
    public function editprint($id)
    {
        ...
        return view('receptionist.appointment.letterhead.editprint', compact('patient', 'diagnosis', 'letterhead', 'medicines'));
    }
